...
namespace vEngine
{
	...
	GaussianBlur::GaussianBlur(Engine& game, Camera& camera)
	...
	GaussianBlur::GaussianBlur(Engine& game, Camera& camera, float blurAmount)
		: DrawableComponent(game, camera),
		mEffect(nullptr), mMaterial(nullptr), mSceneTexture(nullptr), mOutputTexture(nullptr), mHorizontalBlurTarget(nullptr), mVerticalBlurTarget(nullptr), mFullScreenQuad(nullptr),
	...
	void GaussianBlur::Draw(const Time& gameTime)
	{
	...
	void GaussianBlur::DrawToTexture(const Time& gameTime)
	{
		if (mBlurAmount > 0.0f)
		{
			// Horizontal blur
